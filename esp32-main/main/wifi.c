...
static wifi_on_connecting_cb_t on_connecting_cb = NULL;
...
void wifi_set_on_connecting_cb(wifi_on_connecting_cb_t cb)
{
    on_connecting_cb = cb;
}
...
{
    ESP_LOGI(TAG, "Setup Connecting to SSID %s", ssid);
...
int wifi_connect_new(const char *ssid, const char *password,
                     eap_method_t eap_method, const char *eap_identity,
                     const char *eap_username, const char *eap_password,
                     const char *ca_cert, const char *client_cert, const char *client_key)
{
