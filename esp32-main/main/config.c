...
{
    long heap_free = (unsigned long)esp_get_free_internal_heap_size();
    ESP_LOGI(TAG, "Heap Free size: %lx", heap_free);

    cJSON *JSON_OBJ = (cJSON *)pvParameters;
    if (!JSON_OBJ)
    {
    ...
    char full_path[200];
    snprintf(full_path, sizeof(full_path), "%s%s", conf.base_path, path);

    char *buffer = cJSON_PrintUnformatted(JSON_OBJ);
    if (!buffer)
    {
    ...
    int fd = open(full_path, O_WRONLY | O_TRUNC | O_CREAT);
    if (fd < 0)
    {
    ...
    {
        ESP_LOGE(TAG, "Failed to write to file: %s (%s)", full_path, strerror(errno));
        close(fd);
        free(buffer);
        return NULL;
    }
    ...
    ESP_LOGI(TAG, "JSON saved successfully to %s", full_path);
    return JSON_OBJ; // Returning the input JSON object
}
